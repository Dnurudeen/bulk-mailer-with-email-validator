<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('campaigns.store') }}" class="bg-white p-6 rounded shadow space-y-4">
                @csrf
                <div>
                ...
                <div>
                    ...
                    <textarea name="html_body" rows="8" class="w-full border rounded p-2" required><h1>Hello!</h1><p>Your content here.</p></textarea>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block font-medium">Batch Size</label>
                        <input name="batch_size" type="number" value="100" class="w-full border rounded p-2">
                    </div>
                    <div>
                        <label class="block font-medium">Schedule At (optional)</label>
                        <input name="scheduled_at" type="datetime-local" class="w-full border rounded p-2">
                    </div>
                </div>
                <div class="mt-4 d-flex justify-content-end">
                    <button
                        class="px-4 py-2 bg-blue-600 btn btn-dark rounded">Save</button>
                </div>
            </form>
        </div>
    </div>
